...
class TrainDataset(data.Dataset):
    def __init__(self, city, typo, seq_len, pre_len, num_days, if_norm, if_lack):
        ...
        if city=='bj':
            data = data[:, :400]
        if city == 'sh':
            data = data[:num_days*288, 0:400]
            
        self.mean, self.max, self.min, self.std, self.median, self.numnodes = data.mean(), data.max(), data.min(), data.std(), np.median(data), data.shape[1]
