...
tgt_num_days = 1
yitaloss =0
# ori_environ = str(pre_len*5)+'min_'+'MODLE_'+'FT:'+str(yitaloss)
# ori_environ='MODLE_'+'FT:'+str(yitaloss)+'_'+str(pre_len*5)+'min'
ori_environ='MODLE_'+'FT:'+str(yitaloss)+'_'+str(pre_len*5)+'min_'+'numcluster:'+str(sub_graph_num)
environ=ori_environ+'_retrain'
# environ = 'test'

#===== data load: =====# 
src_speed = TrainDataset(city=tgt_city, typo='speed', seq_len=seq_len, pre_len=pre_len, num_days=tgt_num_days, if_norm=if_norm, if_lack=yitaloss)
src_loader = DataLoader(src_speed, batch_size=load_batch_size, pin_memory=True, num_workers=0, shuffle=True)
tgt_speed = ValDataset(city=tgt_city, typo='speed', seq_len=seq_len, pre_len=pre_len, num_days=tgt_num_days, if_norm=if_norm, if_lack=yitaloss)
...
src_adj = pd.read_csv('./src_data/'+tgt_city+'_adj.csv', header=None).values
...
# src_nodes_part, src_min_num = graph_part('sh_train',src_adj, (src_num_nodes//tgt_min_num)-1)
src_nodes_part, src_min_num = tgt_nodes_part, tgt_min_num
...
if os.path.exists(cwd+'/pkl/'+ori_environ+'.pkl'):
    checkpoint = torch.load(cwd+'/pkl/'+ori_environ+'.pkl')
...
#         src_pre = torch.cat((src_pre1, src_pre2, src_pre3), 0)
    ...
    ### Training
        ...
        for i in range(len(src_nodes_part)):
    ...
    with torch.no_grad():
        net.eval()
        tgt_val_speed = ValDataset(city=tgt_city, typo='speed', seq_len=seq_len, pre_len=pre_len, num_days=tgt_num_days, if_norm=if_norm, if_lack=yitaloss)
